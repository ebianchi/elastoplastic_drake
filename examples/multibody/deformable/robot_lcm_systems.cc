/* Copied over from dairlib with minimal edits.  Commented out unused
definitions, leaving only RobotOutputSender, RobotInputReceiver, and
AddActuationRecieverAndStateSenderLcm.
https://github.com/DAIRLab/dairlib/blob/bibit/deform/systems/robot_lcm_systems.cc
*/

#include "robot_lcm_systems.h"
#include <iostream>

#include "drake/lcmt_robot_input.hpp"
#include "drake/lcmt_robot_output.hpp"
...
void RobotOutputSender::Output(const Context<double>& context,
                               drake::lcmt_robot_output* state_msg) const {
...
RobotInputReceiver::RobotInputReceiver(
  ...
  this->DeclareAbstractInputPort("lcmt_robot_input",
                                 drake::Value<drake::lcmt_robot_input>{});
  this->DeclareVectorOutputPort(
...
void RobotInputReceiver::CopyInputOut(const Context<double>& context,
  ...
  const drake::AbstractValue* input = this->EvalAbstractInput(context, 0);
  DRAKE_ASSERT(input != nullptr);
  const auto& input_msg = input->get_value<drake::lcmt_robot_input>();
...
SubvectorPassThrough<double>* AddActuationRecieverAndStateSenderLcm(
  ...
  auto input_sub =
      builder->AddSystem(LcmSubscriberSystem::Make<drake::lcmt_robot_input>(
  ...
  auto state_pub =
      builder->AddSystem(LcmPublisherSystem::Make<drake::lcmt_robot_output>(
