...
https://github.com/DAIRLab/dairlib/blob/bibit/deform/multibody/multibody_utils.cc
*/
...
map<string, int> MakeNameToPositionsMap(const MultibodyPlant<T>& plant,
  ...
  // if index has not already been captured, throw an error
  DRAKE_THROW_UNLESS(plant.num_positions(model_instance) ==
                     static_cast<int>(index_set.size()));
...
map<string, int> MakeNameToVelocitiesMap(const MultibodyPlant<T>& plant) {
  ...
  // if index has not already been captured, throw an error
  DRAKE_THROW_UNLESS(plant.num_velocities() ==
                     static_cast<int>(index_set.size()));
  return name_to_index_map;
}
...
map<string, int> MakeNameToVelocitiesMap(const MultibodyPlant<T>& plant,
  ...
  // if index has not already been captured, throw an error
  DRAKE_THROW_UNLESS(plant.num_velocities(model_instance) ==
                     static_cast<int>(index_set.size()));
  return name_to_index_map;
}
